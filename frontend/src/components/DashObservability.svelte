<script lang="ts">
  /**
   * DashObservability — Migrated from agent-os ObservabilityPanel.tsx
  ...
  onMount(() => {
    ...
    const unsub = on("observability.event", (data: any) => {
      if (data) {
    ...
    const unsub2 = on("observability.status", (data: any) => {
